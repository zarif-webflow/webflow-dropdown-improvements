...
const initDropdownImprovements = () => {
  ...
  const windowSpecifiedMargin = "globalDropdownMargin" in window && typeof window.globalDropdownMargin === "string" ? window.globalDropdownMargin : DEFAULT_MARGIN;
  ...
  for (const dropdownElement of webflowDropdownElements) {
    ...
    const dropdownMargin = dropdownElement.getAttribute("data-dropdown-margin") || windowSpecifiedMargin || DEFAULT_MARGIN;
    const setModalPosition = () => {
      computePosition(dropdownToggle, dropdownList, {
        ...
        setStyle(dropdownList, {
          top: `${y}px`,
          transform: placement.includes("bottom") ? `translateY(${dropdownMargin})` : `translateY(calc(-1 * ${dropdownMargin}))`
        });
      });
    };
